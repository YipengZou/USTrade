...
class RNNDataset(Dataset):
    def __init__(self, factor_data: pd.DataFrame, label_data: pd.Series):
        ...
        feature_cols = ut().find_pattern_cols(factor_data.columns, 'f_')
        ...
        self.data = factor_data.groupby(['stock_id']).\
            ...
            )).apply(pd.Series).stack().reset_index(drop = True) # type: ignore
...
class FullyConnectedNet(nn.Module):
    def __init__(
            self, input_size = 64, hidden_sizes: List[int] = [64, 128], 
            output_size = 1, dropout_prob: float = 0.1,
    ) -> None:
        super().__init__()
        self.model_name = 'FCN'
...
class LSTMNet(nn.Module):
    def __init__(
            ...
            num_layers: int, device, output_dim: int = 1
    ):
        super().__init__()
...
class NNModel():
    def __init__(
        ...
        self.save_folder = f'../models/{self.model_name}'
        if not os.path.exists(self.save_folder):
            os.makedirs(self.save_folder)
        self.model_path = f'{self.save_folder}/model.pth'
...
class SimpleNNModel(NNModel):
    def __init__(
        ...
        super().__init__(xcols, ycol, 'SimpleNN')
        self.epoch = epoch
    ...
    def train(
        ...
        for epoch in tqdm(range(self.epoch)):
            ...
            es(valid_loss, self.model, model_path = f'{self.save_folder}/model.pth')
            if es.early_stop:
    ...
    def save_model(self):
        info_path = f'{self.save_folder}/info.json'
...
class LSTMModel(NNModel):
    def __init__(
        ...
        super().__init__(xcols, ycol, 'LSTM')
        self.epoch = epoch
        self.batch_size = batch_size
        self.learning_rate = learning_rate
        self.input_size = input_size
    ...
    def train(
        ...
        for epoch in tqdm(range(self.epoch)):
            ...
            es(valid_loss, self.model, model_path = self.model_path)
            if es.early_stop:
                print("Early stopping")
                break
    ...
    def predict(self, X: pd.DataFrame) -> pd.Series:
        ...
        self.model.eval()
        self.valid_loader = DataLoader(
            RNNDataset(X, pd.Series(np.zeros(len(X)))), batch_size = self.batch_size
        )
        preds, labels = self.inference_fn()
        print(preds.shape)
        print(preds)
